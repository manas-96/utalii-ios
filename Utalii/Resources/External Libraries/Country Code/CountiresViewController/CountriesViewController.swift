...
//Serach bar method to search countries
    ...
    fileprivate func setupCountries() {
        ...
        unfilteredCountries = partionedArray(Stripe ? Countries.countries : countries_all.countries, usingSelector: #selector(getter: NSFetchedResultsSectionInfo.name))
        unfilteredCountries.insert(Stripe ? Countries.countriesFromCountryCodes(majorCountryLocaleIdentifiers):countries_all.countriesFromCountryCodes(majorCountryLocaleIdentifiers), at: 0)
        tableView.reloadData()
...
//section are prepared as per localization of countries
    ...
    public class func show(countriesViewController coVar: CountriesViewController, toVar: UIViewController,stripe_status:Bool) {
        
        let navController  = UINavigationController(rootViewController: coVar)
        coVar.Stripe = stripe_status
