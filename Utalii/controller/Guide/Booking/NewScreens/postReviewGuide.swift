//
//  postReviewGuide.swift
//  Utalii
//
//  Created by Arijit Das on 28/02/24.
//
...
class postReviewGuide: UIViewController {
    ...
    func callGetReview(){
        ...
        let dict = ["orderId":self.marrOrderDetails.guideId,"touristId":self.marrOrderDetails.travellerUserId]

        APIhandler.SharedInstance.API_WithParameters(aurl: API_ReviewList_guide, parameter: dict, completeBlock: { (responseObj) in
            if(showLog){
            ...
            if(self.arrOtherReviewModel.status.equalIgnoreCase("1")){
                print("INSIDE aray count is \(self.arrOtherReviewModel.reviewList?.count)")
    ...
    func calPostReview(){
        ...
        let dict = ["orderId":self.marrOrderDetails.orderId,"touristId":self.marrOrderDetails.travellerUserId,"guideId":UserDefaults.standard.string(forKey: KU_USERID) ?? "","rating":"\(rating)","comments":txtReview.text ?? ""]

        APIhandler.SharedInstance.API_WithParameters(aurl: API_AddReview_Guide, parameter: dict, completeBlock: { (responseObj) in
            if(showLog){
                print("\(dict)  -- \(responseObj)")
            }
            let apiResponce = JSON(responseObj)
    ...
    func setTheme(){
        ...
        let urlNew:String = (marrOrderDetails.travellerPic).replacingOccurrences(of: " ", with: "%20")
        imgGuide.sd_setImage(with: URL(string: urlNew), placeholderImage: UIImage(named: "loding.png"))
        lblGuideName.text = self.arrOtherReviewModel.avg//marrOrderDetails.guidefullName
        lblKnowLanguage.text = marrOrderDetails.languageKnown
        viewGuideRate.rating = Double(self.arrOtherReviewModel.avg) ?? 0.0//Double(marrOrderDetails.avgRating) ?? 0.0
...
extension postReviewGuide: UITableViewDelegate,UITableViewDataSource{
