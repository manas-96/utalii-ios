//
//  EditGuideDetailVC.swift
//  Utalii
//
//  Created by Mitul Talpara on 18/01/23.
//
...
class EditGuideDetailVC: UIViewController,UIImagePickerControllerDelegate,changeDataDeleget,GMSMapViewDelegate, UINavigationControllerDelegate,stripe_Details {
    ...
    override func viewDidLoad() {
       ...
       // txtEmail.text = email
        ...
        if(UserDefaults.standard.string(forKey: KU_STRIPECONNECTSTATUS) == "1" ){
            self.StripeConnectButtion.setTitle("Stripe Details", for: .normal)
...
func getStripeURL(){
    ...
    func setTheme(){
     ...
     //   self.imagePicker = ImagePicker(presentationController: self, delegate: self)
        ...
        arrSelectedItemID = data?.interestListID ?? [""]
        
        print("The ITEM ARRAY IS \(arrSelectedItemID)")
...
//        }
        ...
        // sid
          ...
          print("The Dates received are \(dates)")
        if(dates == "00-00-00"){
            txtBdate.text = ""
...
//
    ...
    func callSaveDataAPI(type: Int) {
        ...
        let mobile = "+" + countryCode + " " + (txtPhone.text ?? "")
        
        // Prepare date of birth
        let dob = self.dob.isEmpty ? (data?.dob ?? "") : self.dob
        
        // Prepare parameters
        let parameters: [String: Any] = [
            "guideId": UserDefaults.standard.string(forKey: "KU_USERID") ?? "",
            ...
            "languageKnown": arrSelectedLanguage.joined(separator: ","),
            "placesKnown": arrImportantplaces.joined(separator: ","),
            "interest": arrSelectedItemID.joined(separator: ","),
            "dob": dob,
            "since": txtGuidAge.text ?? "",
            "minimumCharge": txtMinimumBooking.text ?? "",
            "tourStartTime": txtStartTime.text ?? "",
            "permanentAddress": txtStreetAddtess.text ?? "",
            "city": txtCity.text ?? "",
            "state": txtState.text ?? "",
        ...
        guard let coverImage = imgCover.image,
              let documentImage = imgDocument.image,
              let profileImage = imgPhoto.image else {
            IJProgressView.shared.hideProgressView()
            ConstantFunction.sharedInstance.showAlert(aTitle: "AppName", aMsg: "Please select all required images")
            return
        }
        
        // Call the API with three images
        APIhandler.SharedInstance.Api_withImageAndParameters(
            aWebServiceName: "YOUR_API_URL_HERE", // Replace with your API URL
            aDictParam: parameters,
            aImag: coverImage,
            aImag2: documentImage,
            aImag3: profileImage,
            imgParameter: "coverPic",
            imgParameter2: "license",
            imgParameter3: "profilePic",
            completeBlock: { responseObj in
                IJProgressView.shared.hideProgressView()
                    ...
                    if let status = json["status"] as? String,
                       status == "1" {
                        ...
                        if let message = json["message"] as? String {
                            ConstantFunction.sharedInstance.showAlert(aTitle: "AppName", aMsg: message)
                        }
                        self.navigationController?.popViewController(animated: true)
                    } else {
                        // Error
                        let errorMsg = json["error"] as? String ?? "Unknown error"
                        ConstantFunction.sharedInstance.showAlert(aTitle: "AppName", aMsg: errorMsg)
                    }
                }
            },
            errorBlock: { errorObj in
                IJProgressView.shared.hideProgressView()
                
                // Handle error
                if let errorDict = errorObj as? [String: Any],
                   let errorMsg = errorDict["errormsg"] as? String {
                    ConstantFunction.sharedInstance.showAlert(aTitle: "AppName", aMsg: errorMsg)
                } else {
                    ConstantFunction.sharedInstance.showAlert(aTitle: "AppName", aMsg: "Upload failed. Please try again.")
                }
            }
        )
    }
...
extension EditGuideDetailVC: UITableViewDelegate,UITableViewDataSource{
...
extension EditGuideDetailVC: GMSAutocompleteViewControllerDelegate {
...
extension EditGuideDetailVC: CountriesViewControllerDelegate {
